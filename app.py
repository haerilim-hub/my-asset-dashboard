...
# ==============================================================================
# [PAGE 2] 데이터 입력 도우미 (콤마 기능 강화)
# ==============================================================================
elif menu == "📝 데이터 입력 도우미":
    st.title("📝 간편 데이터 생성기")
    st.info("💡 위쪽 표에서 콤마(,)가 포함된 금액을 수정하면, 아래쪽에서 자동 계산됩니다.")
    ...
    if input_password != ADMIN_PASSWORD:
        ...
        # -------------------------------------------------------------
        # 1. 콤마가 보이도록 숫자를 문자로 변환 (전처리)
        # -------------------------------------------------------------
        # 원금과 평가액을 '1,000,000' 형태의 글자로 바꿉니다.
        ...
        input_df['원금'] = input_df['원금'].apply(format_with_comma)
        input_df['평가액'] = input_df['평가액'].apply(format_with_comma)

        st.subheader("1️⃣ 금액 수정 (입력용)")
        st.caption("👇 이제 콤마(,)를 보면서 편하게 수정하세요.")
        
        editable_cols = ['주체', '증권사', '구분', '종목명', '테마', '원금', '평가액']
        
        # TextColumn으로 설정하여 콤마 입력을 허용함
        edited_df = st.data_editor(
            input_df[editable_cols],
            num_rows="dynamic",
            use_container_width=True,
            column_config={
                "원금": st.column_config.TextColumn(validate="^[0-9,-]+$"), # 숫자,콤마,마이너스만 입력 가능
                "평가액": st.column_config.TextColumn(validate="^[0-9,-]+$"),
            }
        )
        
        # -------------------------------------------------------------
        # 2. 계산을 위해 다시 숫자로 변환 (후처리)
        # -------------------------------------------------------------
        # 콤마를 떼고 숫자로 바꿔주는 함수
        def clean_currency(x):
            try:
                # 문자열로 바꾸고 콤마 제거 후 실수로 변환
                return float(str(x).replace(',', '').replace('(', '-').replace(')', ''))
            except:
                return 0.0

        # 계산용 숫자 컬럼 생성
        edited_df['평가액_num'] = edited_df['평가액'].apply(clean_currency)
        edited_df['원금_num'] = edited_df['원금'].apply(clean_currency)
        ...
        # 미리보기용 데이터프레임 (다시 콤마 찍어서 보여줌)
        preview_df = edited_df[['종목명', '원금_num', '평가액_num', '평가손익']].copy()
        preview_df.columns = ['종목명', '원금', '평가액', '평가손익'] # 이름 깔끔하게
        ...
        if st.button("🚀 위 내용으로 데이터 생성하기"):
            # 내보낼 때는 콤마 없는 깨끗한 숫자로 내보내야 구글 시트가 좋아합니다.
            ...
            try:
                ...
                st.success("✅ 데이터 생성 완료! 복사해서 붙여넣으세요.")
                # 복사할 데이터 (탭으로 구분)
