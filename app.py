...
@st.cache_data(ttl=60) # 60초마다 캐시 갱신 (새로고침 시 반영)
def load_data(url):
    try:
        if "/d/" in url:
...
# --- 메인 화면 ---
st.set_page_config(layout="wide", page_title="내 투자 자산 대시보드")
...
if error_msg:
    ...
    if selected_subject != '전체':
        df = df[df['주체'] == selected_subject]
        st.sidebar.info(f"👉 '{selected_subject}' 자산만 분석합니다.")
    ...
    with tab1:
        ...
        daily_df = df[df['기준일자'] == latest_date].copy()
        
        title_prefix = selected_subject if selected_subject != '전체' else "전체"
        st.title(f"📊 {title_prefix} 자산 현황 ({latest_date.strftime('%Y-%m-%d')})")
    ...
    with tab2:
        st.title("📈 자산 성장 그래프")
        timeline = df.groupby('기준일자')[['평가액', '원금']].sum().reset_index()
        st.plotly_chart(px.line(timeline, x='기준일자', y=['평가액', '원금'], markers=True), use_container_width=True)
        st.plotly_chart(px.area(df, x='기준일자', y='평가액', color='테마'), use_container_width=True)
